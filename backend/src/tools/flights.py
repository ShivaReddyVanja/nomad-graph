...
def search_transit(origin: str, destination: str, start_date: str) -> List[TransitOption]:
    ...
    Uses a 30-second timeout and raises errors on failure or if no results are found.
    """
    ...
    for attempt in range(attempts):
        try:
            ...
            print(f"[Flights Tool] Attempt {attempt + 1} failed: {e}.")
            if attempt < attempts - 1:
                import time
                time.sleep(1)
                continue
            raise RuntimeError(f"[Flights Tool] Failed after {attempts} attempts to query flights. Last error: {e}") from e
    ...
    if not transit_options:
        print(f"[Flights Tool] No flight options found between '{origin}' ({origin_code}) and '{destination}' ({dest_code}).")
        return []
...
def resolve_nearest_airport(city_name: str) -> NearestAirportResolution:
    """
    Resolves the nearest major commercial airport for a city that doesn't have its own direct commercial flights.
    """
    if not city_name:
        raise ValueError("City name is required to resolve nearest airport.")
        
    query = f"nearest major commercial airport to {city_name} with commercial passenger flights"
    system_prompt = (
        "You are a travel database helper. Given a city or region, identify the nearest major "
        "commercial airport that has regular scheduled passenger flights. "
        "Provide the airport city name and its 3-letter IATA code."
    )
    extraction_prompt = f"Identify the nearest major commercial airport to: '{city_name}'"
    
    return search_and_extract_fact(
        query=query,
        system_prompt=system_prompt,
        extraction_prompt=extraction_prompt,
        output_schema=NearestAirportResolution
    )
